...
void SettingsDialog::createSettingsGroup() {
    ...
    // 修改一天时间的设置
    m_dayDurationSpinBox->setRange(240, 1440);  // 从240秒到1440秒(24分钟)
    m_dayDurationSpinBox->setSingleStep(24);    // 每次增加24秒
    m_dayDurationSpinBox->setSuffix(" 秒");
    m_dayDurationSpinBox->setToolTip("设置一天的模拟时间长度（必须是24的倍数）");

    m_idleTimeSpinBox->setRange(5, 30);
    m_idleTimeSpinBox->setSuffix(" 秒");
    m_idleTimeSpinBox->setToolTip("设置电梯空闲等待时间");

    m_requestIntervalSpinBox->setRange(5, 30);
    m_requestIntervalSpinBox->setSuffix(" 秒");
    m_requestIntervalSpinBox->setToolTip("设置随机乘客生成间隔");

    m_randomPassengerSpinBox->setRange(1, 5);
    m_randomPassengerSpinBox->setSuffix(" 人");
    m_randomPassengerSpinBox->setToolTip("设置每次生成的随机乘客数量");

    m_peakPassengerSpinBox->setRange(10, 30);
    m_peakPassengerSpinBox->setSuffix(" 人");
    m_peakPassengerSpinBox->setToolTip("设置高峰期的乘客数量");

    m_doorTimeSpinBox->setRange(100, 2000);
...
void SettingsDialog::onDayDurationChanged(int value) {
    if (value % 24 != 0) {
        // 自动调整到最近的24的倍数
        int adjustedValue = (value / 24) * 24;
        m_dayDurationSpinBox->setValue(adjustedValue);
        m_errorLabel->setText("一天时间已自动调整为24的倍数");
    } else {
        m_errorLabel->clear();
        validateInput();
    }
