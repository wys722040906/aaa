...
void ElevatorDispatcher::initializeElevators() {
    // 初始化4个电梯
    for (int i = 0; i < 4; ++i) {
        ...
        if (i == 0) {
            ...
            config.minFloor = Config::MIN_FLOOR;
            config.maxFloor = Config::MAX_FLOOR;
            config.doorTime = 0.5;
            config.moveTime = Config::DEFAULT_SPEED;
            elevator->setConfig(config);
        } else {
            // 其他电梯使用默认配置
            Elevator::Config config;
...
void ElevatorDispatcher::setElevatorSpeed(int elevatorId, double speed) {
    if (elevatorId >= 0 && elevatorId < elevators.size()) {
        if (auto elevator = elevators[elevatorId]) {
            Elevator::Config config = elevator->getConfig();
            config.moveTime = speed;
            elevator->setConfig(config);
        }
    }
}