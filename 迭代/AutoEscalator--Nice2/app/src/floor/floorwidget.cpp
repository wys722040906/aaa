...
void FloorWidget::clearPassengerRequests() {
    ...
    setStatus(false);  // 设置为空闲状态
    showRequestList(false);  // 隐藏请求列表
    updateRequestList();  // 更新显示
    m_statusBall->update();  // 强制更新状态球显示
}
...
void FloorWidget::updateRequestList() {
    ...
    // 添加标题行
    QListWidgetItem* titleItem = new QListWidgetItem("目标楼层  数");
